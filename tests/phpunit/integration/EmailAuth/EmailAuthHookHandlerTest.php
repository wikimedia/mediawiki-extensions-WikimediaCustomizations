...
namespace MediaWiki\Extension\WikimediaCustomizations\Tests\EmailAuth;
...
class EmailAuthHookHandlerTest extends MediaWikiIntegrationTestCase {
	...
	private IPReputationIPoidDataLookup&MockObject $ipReputationDataLookup;
	private LoginNotify&MockObject $loginNotify;
	private WikimediaEventsCountryCodeLookup&MockObject $countryCodeLookup;
	private LoggerInterface&MockObject $logger;

	private FauxRequest $request;
	private User&MockObject $user;
	private EmailAuthHookHandler $hookHandler;
	...
	public function testShouldDoNothingIfExtensionDependenciesNotMet(): void {
		...
		$res = $this->hookHandler->onEmailAuthRequireToken(
			$this->user,
	...
	public function testShouldRequireVerificationForNon2FAUsersOnUnknownIPsKnownToIpoid(
		...
		$this->ipReputationDataLookup->method( 'getIPoidDataForIp' )
			->with( '100.101.102.103', EmailAuthHookHandler::class . '::onEmailAuthRequireToken' )
	...
	public static function provideVerificationCases(): iterable {
		...
		foreach ( $testCases as $params ) {
			...
			$description = sprintf(
				'2FA %s, %s, %s, %s, $wgWMCEmailAuthEnforce: %s, LoginNotify status: %s, %s',
	...
	public function testGetPrivilegedGroupsCallback() {
		...
		$verificationRequired = false;
		$this->hookHandler->onEmailAuthRequireToken(
			$this->user,
			$verificationRequired,
			$formMessage,
			$subject,

 ...
 * @file
 */
namespace MediaWiki\Extension\WikimediaCustomizations\RestSandbox;
...
class SpecialRestSandbox extends SpecialPage {
	...
	public function execute( $subPage ) {
		...
		$out->addModuleStyles( [
			'mediawiki.special',
			'mediawiki.hlist',
			'ext.wikimediaCustomizations.special.restsandbox.styles'
		] );

		if ( !$apiSpecs ) {
			$out->addHTML( Html::errorBox(
				$out->msg( 'wikimediacustomizations-restsandbox-no-specs-configured' )->parse()
			) );
			return;
		}

		if ( $out->getLanguage()->getCode() !== 'en' ) {
			$out->addHTML( Html::noticeBox( $out->msg( 'wikimediacustomizations-restsandbox-disclaimer' )->parse() ) );
		}
		...
		if ( !$specUrl ) {
			$out->addHTML( Html::errorBox(
				$out->msg( 'wikimediacustomizations-restsandbox-no-such-api', $apiId )->parse()
			) );
			return;
		}
		...
		$out->addModules( [
			...
			'ext.wikimediaCustomizations.special.restsandbox'
		] );
		...
		$out->addHTML( Html::errorBox(
			$out->msg( 'wikimediacustomizations-restsandbox-jsonly' )->parse(),
			'',
	...
	private function showForm( array $apiSpecs, string $apiId ) {
		...
		$formDescriptor = [
			'intro' => [
				'type' => 'info',
				'raw' => true,
				'default' => $this->msg( 'wikimediacustomizations-restsandbox-text' )->parseAsBlock()
			],
			'api' => [
				'type' => 'select',
				'name' => 'api',
				'label-message' => 'wikimediacustomizations-restsandbox-select-api',
				'options' => $apis,
				'default' => $apiId
			],
			'title' => [
